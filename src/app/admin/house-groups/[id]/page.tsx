    ...
    if (error || !group) {
        return (
            <div className="flex h-full flex-col items-center justify-center space-y-4 py-12">
                <h2 className="text-lg font-semibold text-destructive">House group not found</h2>
                <Button onClick={() => router.push("/admin/house-groups")}>
                    <ArrowLeft className="mr-2 h-4 w-4" />
                    Back to House Groups
                </Button>
            </div>
        );
    }

    const houses = group.houses || [];
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => router.push("/admin/house-groups")}
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => router.push(`/admin/houses?houseGroupId=${group.id}`)}>
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="pt-4 border-t border-border/50 space-y-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Badge variant="default" className="font-mono">{houses.length}</Badge>
                                </div>
                                <div className="flex items-center justify-between">
                ...
                <Card className="md:col-span-2 shadow-sm border-border/50">
                    <div className="p-6 border-b border-border/50 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-semibold text-lg leading-tight text-foreground">Member Houses</h3>
                                <p className="text-xs text-muted-foreground mt-0.5">Houses currently assigned to this group</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="p-4">
                            {houses.length > 0 ? (
                                <DataTable
                                    ...
                                    columns={columns}
                                    searchable={true}
                                    searchPlaceholder="Search houses in group..."
                                    getRowId={(row) => row.id}
                                    pageSize={10}
                                    showPagination={houses.length > 10}
                                ...
                                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                                    ...
                                    <div className="max-w-[280px]">
                                        <p className="font-semibold text-lg">No houses assigned</p>
                                        <p className="text-sm text-muted-foreground mt-1">
                                            This group is empty. You can assign houses to it from the Houses management page.
                                        </p>
                                    </div>
                                    <Button variant="outline" size="sm" onClick={() => router.push("/admin/houses")}>
                                        Assign Houses
                                    </Button>
                                </div>
                            )}
