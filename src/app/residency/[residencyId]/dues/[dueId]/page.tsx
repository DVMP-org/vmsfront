...
export default function ResidentDueDetailPage() {
    const params = useParams<{ residencyId: string; dueId: string }>();
    const router = useRouter();
    const residencyId = params?.residencyId ?? null;
    const dueId = params?.dueId ?? null;

    const { data: residencyDue, isLoading } = useResidencyDue(residencyId, dueId);
    ...
    const scheduleMutation = useScheduleResidencyDue(residencyId, dueId);
    const payScheduleMutation = usePayDueSchedule(residencyId, dueId);
    ...
    useEffect(() => {
        if (transaction && paymentReference) {
            if (transaction.status === "success") {
                ...
                queryClient.invalidateQueries({ queryKey: ["resident", "due-schedules", residencyId, dueId] });
                queryClient.invalidateQueries({ queryKey: ["resident", "due-payments", residencyId, dueId] });
            } else if (transaction.status === "failed") {
                toast.error("Payment failed. Please try again.");
                setPaymentReference(null);
            }
        }
    }, [transaction, paymentReference, queryClient, residencyId, dueId]);
    ...
    if (!residencyDue) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <Button variant="outline" className="mt-6" onClick={() => router.push(`/residency/${residencyId}/dues`)}>
                    Back to Dues
                </Button>
            </div>
        );
    ...
    const { due } = residencyDue;
    const isActivated = residencyDue.payment_breakdown !== null;
    ...
    return (
        <div className="space-y-6 max-w-7xl animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between">
                <button
                    onClick={() => router.push(`/residency/${residencyId}/dues`)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <FinancialMetric
                    label="Total Amount"
                    value={formatCurrency(residencyDue.amount)}
                    subValue="Due for this unit"
                    icon={CreditCard}
                />
                <FinancialMetric
                    label="Amount Paid"
                    value={formatCurrency(residencyDue.paid_amount)}
                ...
                <FinancialMetric
                    ...
                    value={formatCurrency(residencyDue.balance)}
                    subValue="Outstanding amount"
                    icon={Wallet}
                // className={residencyDue.balance > 0 ? "text-red-500" : "text-emerald-600"}
                />
            </div>
                    ...
                    <div className="space-y-3">
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Badge variant={
                                    residencyDue.status == ResidencyDueStatus.PAID ? "secondary" : ResidencyDueStatus.PARTIALLY_PAID ? "warning" : "danger"
                                }>
                                    {titleCase(residencyDue.status)}
                                </Badge>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-6">
                            ...
                            <div>
                                ...
                                <p className="text-xs font-medium">{titleCase(residencyDue?.payment_breakdown)}</p>
                            </div>
                            <div>
                                <p className="text-[10px] uppercase font-bold text-muted-foreground/60 mb-1">Next Schedule Date</p>
                                <p className="text-xs font-medium">{formatDateTime(residencyDue?.next_schedule?.payment_date || "")}</p>
                            </div>
                    ...
                    <Card className="rounded-lg shadow-none border-brand-primary border-2 bg-brand-primary/[0.02] overflow-hidden">
                        ...
                        <CardContent className="p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {residencyDue.payment_breakdown_options?.map((option) => (
                                    <button
