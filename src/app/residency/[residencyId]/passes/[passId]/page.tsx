...
export default function PassDetailPage() {
  const params = useParams<{ residencyId?: string | string[]; passId?: string | string[] }>();
  ...
  const { data: pass, isLoading } = useGatePass(residencyId, passId);
  const revokePassMutation = useRevokeGatePass(residencyId);
  const removeVisitorMutation = useRemoveVisitorFromGatePass(residencyId);
  ...
  const handleRevoke = () => {
    if (passId && confirm("Are you sure you want to revoke this pass?")) {
      revokePassMutation.mutate(passId, {
        onSuccess: () => {
          router.push(`${residencyBase}/passes`);
        },
      });
    }
  };
      ...
      <>
        <div className="border border-muted rounded bg-white p-8">
          <EmptyState
            ...
            description="Choose a residency from the dashboard selector to view pass details."
            action={{
