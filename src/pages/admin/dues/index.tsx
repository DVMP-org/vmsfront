...
import { Plus, Receipt, LayoutGrid, ArrowRight } from "lucide-react";
...
export default function DuesPage() {
    ...
    }, [page, pageSize, search, sort, recurring, tenure_length, startDate, endDate, syncToUrl, router.isReady]);

    const activeFilters = useMemo(() => {
        const filters: FilterConfig[] = [];
        if (recurring !== undefined) filters.push({ field: "recurring", operator: "eq", value: recurring });
        if (tenure_length !== undefined) filters.push({ field: "tenure_length", operator: "eq", value: tenure_length });
        if (startDate !== undefined) filters.push({ field: "created_at", operator: "gte", value: startDate });
        if (endDate !== undefined) filters.push({ field: "created_at", operator: "lte", value: endDate });
        return filters;
    ...
    const availableFilters: FilterDefinition[] = [
        ...
        { field: "created_at", label: "Date", type: "date-range" },
        {
            field: "tenure_length",
            label: "Billing Cycle",
            type: "select",
    ...
    const columns: Column<Due>[] = [
        {
            ...
            accessor: (row) => (
                <div className="py-1">
                    ...
                    <span className="text-[10px] text-muted-foreground font-mono uppercase opacity-70">ID: {row.id.split('-')[0]}</span>
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            accessor: (row) => <span className="text-xs text-muted-foreground capitalize">{titleCase(row.tenure_length)}</span>,
        },
        {
            key: "recurring",
            header: "Plan Type",
        ...
        <div className="space-y-6 max-w-7xl animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-border pb-6">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Manage property fees and dues.</p>
                </div>
                <div className="flex items-center gap-3">
                    <Button variant="outline" size="sm" className="h-10" onClick={() => router.push("/admin/dues/houses")}>
                        <LayoutGrid className="h-4 w-4 mr-2" />
                        House List
                    </Button>
                    <Button size="sm" className="h-10 px-4" onClick={() => router.push("/admin/dues/create")}>
            ...
            <div className="p-3 border border-border/60 rounded-lg overflow-hidden">
                <DataTable
                    ...
                    onFiltersChange={(filters) => {
                        setPage(1);
                        setRecurring(filters.find(f => f.field === "recurring")?.value as any);
                        setTenureLength(filters.find(f => f.field === "tenure_length")?.value as any);
                        setStartDate(filters.find(f => f.field === "created_at" && f.operator === "gte")?.value as any);
                        setEndDate(filters.find(f => f.field === "created_at" && f.operator === "lte")?.value as any);
                    }}
                    onSortChange={(sort) => { setPage(1); setSort(sort); }}
...
DuesPage.getLayout = function getLayout(page: ReactElement) {
    return (
        <RouteGuard>
