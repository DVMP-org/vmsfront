...
export default function ResidentDetailPage() {
    ...
    const residentId = useMemo(() => (Array.isArray(id) ? id[0] : id) ?? "", [id]);

    const { data: residentData, isLoading, error } = useAdminResident(residentId);
    const { data: residentHouses = [], isLoading: housesLoading } = useAdminResidentHouses(residentId);
...
ResidentDetailPage.getLayout = function getLayout(page: ReactElement) {
    return (
        <RouteGuard>
