...
import { ArrowLeft, Ban, Home as HomeIcon, Copy, Check, AlertCircle, Trash2, Plus, Upload } from "lucide-react";
import { GatePassStatus } from "@/types";
import { DashboardLayout } from "@/components/layout/DashboardLayout";
import { RouteGuard } from "@/components/auth/RouteGuard";
import { AddVisitorModal } from "@/components/modules/admin/gate-pass/AddVisitorModal";
import { UploadVisitorsModal } from "@/components/modules/admin/gate-pass/UploadVisitorsModal";
...
export default function PassDetailPage() {
    ...
    const revokePassMutation = useRevokeGatePass(hId);
    const removeVisitorMutation = useRemoveVisitorFromGatePass(hId);
    ...
    useEffect(() => {
        ...
        const match = profile.houses.find((house) => house.id === hId);
        if (match) {
            setSelectedHouse(match);
        }
    }, [hId, profile?.houses, selectedHouse?.id, setSelectedHouse]);

    const houseBase = hId ? `/house/${hId}` : "/select";
    ...
    const handleBulkRemove = () => {
        if (!pId || selectedVisitorIds.length === 0) return;
        setIsDeleteModalOpen(true);
    };
    ...
    const handleRevoke = () => {
        if (pId && confirm("Are you sure you want to revoke this pass?")) {
            revokePassMutation.mutate(pId, {
                onSuccess: () => {
